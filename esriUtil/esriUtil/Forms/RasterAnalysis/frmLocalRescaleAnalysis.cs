...
{
    public partial class frmLocalRescaleAnalysis : Form
    {
        ...
        public frmLocalRescaleAnalysis(IMap map, ref rasterUtil rasterUtility, bool AddToMap)
        {
            InitializeComponent();
        ...
        private void populateComboBox()
        {
            ...
            foreach (string s in Enum.GetNames(typeof(rasterUtil.localRescaleType)))
            {
                cmbFunction.Items.Add(s);
            }
            cmbFunction.SelectedItem = rasterUtil.localRescaleType.PrcTile.ToString();
        }
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            rasterUtil.localRescaleType op = (rasterUtil.localRescaleType)Enum.Parse(typeof(rasterUtil.localRescaleType),funNm);
            this.Visible = false;
            esriUtil.Forms.RunningProcess.frmRunningProcessDialog rp = new RunningProcess.frmRunningProcessDialog(false);
            DateTime dt = DateTime.Now;
            rp.addMessage("Rescaling Rasters. This may take a while...");
            rp.stepPGBar(10);
            rp.TopMost = true;
            rp.Show();
            try
            {
                outraster = rsUtil.localRescalefunction(fdset,op);
