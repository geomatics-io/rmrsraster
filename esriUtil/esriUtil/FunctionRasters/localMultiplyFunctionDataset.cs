    ...
    class localMultiplyFunctionDataset : localFunctionBase
    {
        public override void updateOutArr(ref System.Array outArr, ref List<System.Array> pArr)
        {
            ...
            for (int i = 0; i < pBHeight; i++)
            {
                ...
                {
                    float mult = System.Convert.ToSingle(pArr[0].GetValue(k, i));
                    if (rasterUtil.isNullData(mult, System.Convert.ToSingle(noDataValueArr.GetValue(0))))                    
                    {
                        continue;
                    }
