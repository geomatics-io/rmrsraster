...
{
    class localEntropyFunctionDataset : IRasterFunction
    {
        public localEntropyFunctionDataset()
        {
        }
                ...
                for (int i = pBRowIndex; i < pBHeight; i++)
                {
                    for (int k = pBColIndex; k < pBWidth; k++)
                    {
                        ...
                        for (int nBand = 0; nBand < outPb.Planes; nBand++)
                        {
                            ...
                            int cnt = 0;
                            if (entDic.TryGetValue(pixelValue, out cnt))
                            {
                                cnt = cnt + 1;
                                entDic[pixelValue] = cnt;
                            }
                            else
                            {
                                entDic.Add(pixelValue, 1);
                            }
                        }
                        double entropy = 0;
                        foreach (int entCnt in entDic.Values)
                        {
                            double prob = System.Convert.ToDouble(entCnt)/outPb.Planes;
