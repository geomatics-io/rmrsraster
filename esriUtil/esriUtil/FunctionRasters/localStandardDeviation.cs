...
{
    class localStandardDeviationFunctionDataset : IRasterFunction
    {
        public localStandardDeviationFunctionDataset()
        {
        }
                ...
                for (int i = pBRowIndex; i < pBHeight; i++)
                {
                    for (int k = pBColIndex; k < pBWidth; k++)
                    {
                        ...
                        for (int nBand = 0; nBand < outPb.Planes; nBand++)
                        {
                            ...
                            sumVls += pixelValue;
                            sumVls2 += Math.Pow(pixelValue, 2);
                        }
                        if (!Double.IsNaN(std))
                        {
                            sumVls = Math.Pow(sumVls, 2) / outPb.Planes;
                            variance = (sumVls2 - sumVls) / outPb.Planes;
