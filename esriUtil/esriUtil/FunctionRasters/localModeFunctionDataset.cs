...
{
    class localModeFunctionDataset : IRasterFunction
    {
        public localModeFunctionDataset()
        {
        }
        ...
        public void Read(IPnt pTlc, IRaster pRaster, IPixelBlock pPixelBlock)
        {
            try
            {
                ...
                for (int i = pBRowIndex; i < pBHeight; i++)
                {
                    ...
                    {
                        Dictionary<double,int> modeDic = new Dictionary<double,int>();
                        double mode = noDataVl;
                        int maxCnt = 1;
                        for (int nBand = 0; nBand < outPb.Planes; nBand++)
                        {
                            ...
                            int cnt = 0;
                            if (modeDic.TryGetValue(pixelValue, out cnt))
                            {
                                cnt = cnt + 1;
                                modeDic[pixelValue] = cnt;
                                if (cnt > maxCnt)
                                {
                                    mode = pixelValue;
                                    maxCnt = cnt;
                            ...
                            {
                                if (mode == noDataVl) mode = pixelValue;
                                modeDic.Add(pixelValue, 1);
                            }
                        }
                        outArr.SetValue(mode, k, i);
                    }
                }
