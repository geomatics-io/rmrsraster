...
{
    class neighborhoodHelperMinRectangle
    {
                ...
                for (int nBand = 0; nBand < pbBig.Planes; nBand++)
                {
                    ...
                    for (int j = 0; j < pbBigWd; j++)
                    {
                        ...
                        for (int i = 0; i < rws; i++)
                        {
                            ...
                            if (bVl == noDataValue) bVl = Double.MaxValue;
                            rwsMinArr.Add(bVl);
                        }
                        double rwsMin = rwsMinArr.Min();
                        ...
                        if (cl >= 0)
                        {
                            if (cl > 0)
                            {
                                ...
                                clmsMinArr.Remove(oldClmsValues.Min());

                            }
                            pixelValues.SetValue(clmsMinArr.Min(), cl, 0);
                        }
                    ...
                    for (int i = rws; i < pbBigHt; i++)
                    {
                        ...
                        for (int k = 0; k < clms; k++)
                        {
                            ...
                            if (vlRwT == noDataValue) vlRwT = Double.MaxValue;
                            double bVl = System.Convert.ToDouble(pixelValuesBig.GetValue(k, i));
                            if (bVl == noDataValue) bVl = Double.MaxValue;
                            pRwsMinLst.Remove(vlRwT);
                            pRwsMinLst.Add(bVl);
                            clmsValues[k,rw] = pRwsMinLst;
                            clmsMinArr.Add(pRwsMinLst.Min());
                        }
                        pixelValues.SetValue(clmsMinArr.Min(), 0, rw);
                    }
                        ...
                        for (int k = clms; k < pbBigWd; k++)
                        {
                            ...
                            if(clmp==0)
                            {
                                for(int ki = 0;ki<clms;ki++)
                                {
                                    ...
                                    clmsMinArr.Add(((List<double>)clmsValues[ki,rw]).Min());
                                }
                            ...
                            pRwsMinLst.Remove(vlRwT);
                            pRwsMinLst.Add(bVl);
                            double rwsMin = pRwsMinLst.Min();
                            clmsMinArr.Add(rwsMin);
                            clmsValues[k,rw] = pRwsMinLst;
                            List<double> oldClmsValues = (List<double>)clmsValues[clmp,rw];
                            clmsMinArr.Remove(oldClmsValues.Min());
                            pixelValues.SetValue(clmsMinArr.Min(), cl, rw);
                        }
                    }
