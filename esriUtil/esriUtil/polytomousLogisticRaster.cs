...
namespace esriUtil
{
    ...
    public class polytomousLogisticRaster
    {
            ...
            for (int i = 0; i < oFlds.Length; i++)
            {
                ...
                if (category.ToLower().StartsWith(dependentfield.ToLower().TrimEnd(new char[] { ' ' })))
                {
                    ...
                    if (vl != "" && vl != null && vl != ".")
                    {
                        try
                        {
                            double vld = System.Convert.ToDouble(vl);
                            if (cBetas.TryGetValue(category, out betas))
                            {
                            }
                            else
                            {
                                betas = new Dictionary<string, double>();
                            }
                            betas.Add(param, vld);
        ...
        /// </summary>
        /// <returns>a IRaster Array with the first Raster being the probablity raster and the second raster being the maximum likelihood raster</returns>
